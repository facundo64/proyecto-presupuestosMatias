    ...
    return (
                            ...
                            {item.isPackage ? (
                                ...
                                {item.subItems.map((sub, subIndex) => (
                                    <div key={subIndex} className="grid grid-cols-10">
                                        ...
                                        <p className="col-span-2 text-right font-medium">
                                          {item.showSubItemPrices ? `$${parseFloat(sub.value).toFixed(2)}` : '-'}
                                        </p>
                                    </div>
                                ))}
                                <div className="grid grid-cols-10 pt-2 mt-2 border-t">
                                    ...
                                    <p className="col-span-2 text-right font-bold">
                                      ${item.subItems.reduce((acc, sub) => acc + (parseFloat(sub.value) || 0), 0).toFixed(2)}
                                    </p>
                                </div>
                                </>
                            ) : (
                              <>
                                ...
                                <div className="grid grid-cols-10 pt-2 mt-2 border-t">
                                  <p className="col-span-8 font-semibold text-gray-800">{item.laborType}</p>
                                  <p className="col-span-2 text-right font-bold">
                                    {item.showPrice ? `$${parseFloat(item.value).toFixed(2)}` : '-'}
                                  </p>
                                </div>
                              </>
                            )}
        ...
        <div>
            ...
            <ul className="list-disc list-inside space-y-1 pl-2">
                ...
                <li><span className="font-semibold">Reparación de Fisuras:</span> La garantía sobre reparación de fisuras y grietas cubre únicamente las zonas intervenidas con las técnicas descriptas en el presupuesto (ej. llaves de acero y materiales cementicios). No cubre una garantía total de la pileta ni la aparición de nuevas fisuras en áreas no reparadas.</li>
...
<footer className="text-center mt-12 pt-6 border-t border-gray-200">
    ...
    <p className="text-xs text-gray-400 mt-1">
        Obra Azul Piscinas - (https://obraazulpiscinas.com)
    </p>
</footer>